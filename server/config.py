...
class Config:
    ...
    def _ensure_dirs(self):
        for d in [self.config_dir, self.resource_dir, self.timetable_dir]:
    ...
    def _default_settings(self):
        return {
            ...
            "recorder": {
                ...
                "cameras": {
                    "blackboardL": {
                        "rtsp": "rtsp://admin:password@192.168.1.11:554/cam/realmonitor?channel=1&subtype=0",
                        "enabled": True,
                    },
                    "blackboardR": {
    ...
    def add_user(self, username, password="12345678"):
        ...
        if not username.strip(): return False, "用户名不能为空"
    ...
    def delete_user(self, username):
        ...
        if username == "admin": return False, "不能删除管理员账号"
        del users[username]
        self._save_users_dict(users)
    ...
    def get_timetable_file(self, target_date=None):
        ...
        if target_date is None: target_date = _date.today()
        date_str = target_date.strftime("%Y-%m-%d")
        days = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
        weekday_file = os.path.join(self.timetable_dir, f"{days[target_date.weekday()]}.txt")
        temp_file = os.path.join(self.timetable_dir, f"{date_str}.txt")
        if os.path.exists(temp_file): return temp_file, True
    ...
    def cleanup_temp_timetables(self):
        ...
        if not os.path.exists(self.timetable_dir): return 0
        count = 0
        for fn in os.listdir(self.timetable_dir):
            ...
            try:
                ...
                if file_date < cutoff: os.remove(fp); count += 1
            except ValueError: continue
        return count

    # ======================== 便捷属性 ========================
    @property
    def alias(self): return self.settings["server"]["alias"]

    @property
    def http_port(self): return int(self.settings["server"]["http_port"])

    @property
    def discovery_port(self): return int(self.settings["server"]["discovery_port"])
