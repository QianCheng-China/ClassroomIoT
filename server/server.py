...
class IoTServer:
    ...
    def __init__(self, config):
        ...
        # 【关键修复】强制使用与旧版完全一致的路径计算方式
        # 避免模块化后 config.py 的 __file__ 指向偏差导致 App 读不到板书
    ...
    def _register_routes(self):
        app = self.app
        RESOURCE_DIR = self.RESOURCE_DIR  # 局部变量，方便内部函数使用
        ...
        def get_course_state(describe, date_str, is_today):
            ...
            try:
                ...
                start_dt = datetime.strptime(
                    f"{date_str} {describe.get('start', '00:00')}", "%Y-%m-%d %H:%M")
                end_dt = datetime.strptime(
                    f"{date_str} {describe.get('end', '23:59')}", "%Y-%m-%d %H:%M")
                if now < start_dt:
                    return "future"
                elif now > end_dt:
                    return "finished"
                ...
                for entry in os.listdir(base):
                    if os.path.isdir(os.path.join(base, entry)):
                        try:
                            ...
                            pass
            # 【关键修复】强转 int，防止 JSON 读出字符串导致 timedelta 崩溃返回 500
        ...
        def api_update_check():
            ...
            apk_path = self.config.update_apk
            has_apk = os.path.isfile(apk_path)
            file_size = os.path.getsize(apk_path) if has_apk else 0
            return jsonify({
                "version": desc.get("version", "0.0.0"),
                "changelog": desc.get("changelog", ""),
                "has_apk": has_apk,
                "file_size": file_size,
            })
        ...
        def api_update_download():
            ...
            if not os.path.isfile(apk_path):
                abort(404)
            return send_file(apk_path, as_attachment=True, download_name="classroom_iot.apk")

    # ======================== 服务器启停 ========================

    def start(self):
        """在独立子进程中启动 waitress 服务器"""
        if self.is_running():
            print("[服务器] 已在运行中")
            return
