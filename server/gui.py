...
class AutoStart:
    ...
    @staticmethod
    def is_available(): return sys.platform == "win32"
    ...
    def is_enabled():
        ...
        try:
            import winreg
            key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, AutoStart.REG_PATH)
    ...
    def _get_command():
        if getattr(sys, "frozen", False):
            ...
            exe = sys.executable
            script = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "main.py"))
    ...
    def enable():
        ...
        try:
            import winreg
            key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, AutoStart.REG_PATH, 0, winreg.KEY_SET_VALUE)
            winreg.SetValueEx(key, AutoStart.APP_NAME, 0, winreg.REG_SZ, AutoStart._get_command())
            winreg.CloseKey(key)
            return True
        except Exception: return False
...
class ManagementGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Label(self.root, textvariable=self.status_var, relief="sunken", anchor="w", padding=(8, 4)).pack(fill="x", padx=8, pady=(4, 8))
    ...
    def _build_server_tab(self):
        ...
        lf1.grid(row=row, column=0, sticky="ew", pady=(0, 8)); row += 1
        self.server_status_var = tk.StringVar(value="● 未启动")
        ttk.Label(lf1, textvariable=self.server_status_var, font=("", 11)).grid(row=0, column=0, padx=(0, 16))
        self.btn_start_server = ttk.Button(lf1, text="启动服务器", command=self._start_server)
        self.btn_start_server.grid(row=0, column=1, padx=4)
        self.btn_stop_server = ttk.Button(lf1, text="停止服务器", command=self._stop_server, state="disabled")
        self.btn_stop_server.grid(row=0, column=2, padx=4)

        lf2 = ttk.LabelFrame(tab, text="基本设置", padding=10)
        lf2.grid(row=row, column=0, sticky="ew", pady=(0, 8)); row += 1
        ...
        ttk.Button(lf2, text="保存服务器设置", command=self._save_server_settings).grid(row=3, column=0, columnspan=3, pady=(10, 0))

        lf_autostart = ttk.LabelFrame(tab, text="开机自启动", padding=10)
        lf_autostart.grid(row=row, column=0, sticky="ew", pady=(0, 8)); row += 1
        self.var_autostart = tk.BooleanVar(value=False)
        self.chk_autostart = ttk.Checkbutton(lf_autostart, text="开机时自动启动程序并运行服务器", variable=self.var_autostart, command=self._on_autostart_toggle)
        self.chk_autostart.grid(row=0, column=0, sticky="w")
        if not AutoStart.is_available():
            self.chk_autostart.config(state="disabled")
            ttk.Label(lf_autostart, text="（当前系统不支持，仅限 Windows）", foreground="gray").grid(row=1, column=0, sticky="w", pady=(4, 0))

        lf3 = ttk.LabelFrame(tab, text="用户管理", padding=10)
        lf3.grid(row=row, column=0, sticky="nsew", pady=(0, 8)); row += 1
        tab.rowconfigure(row - 1, weight=1)
        tab.columnconfigure(0, weight=1)
        self.tree_users = ttk.Treeview(lf3, columns=("username",), show="headings", height=6, selectmode="browse")
        ...
        ttk.Button(bf, text="添加用户", command=self._add_user).pack(side="left", padx=4)
        ttk.Button(bf, text="重置选中密码", command=self._reset_password).pack(side="left", padx=4)
        ttk.Button(bf, text="删除选中", command=self._delete_user).pack(side="left", padx=4)
    ...
    def _build_recorder_tab(self):
        ...
        ttk.Label(lf1, textvariable=self.rec_status_var, font=("", 11)).grid(row=0, column=0, padx=(0, 16))
        self.btn_start_rec = ttk.Button(lf1, text="启动录制", command=self._start_recorder)
        self.btn_start_rec.grid(row=0, column=1, padx=4)
        self.btn_stop_rec = ttk.Button(lf1, text="停止录制", command=self._stop_recorder, state="disabled")
        ...
        params = [
            ("多媒体截屏间隔 (秒):", "multimedia_interval", rec["multimedia_interval"]),
            ("摄像头拍照间隔 (秒):", "camera_interval", rec["camera_interval"]),
            ("差异阈值 (RMS):", "diff_threshold", rec["diff_threshold"]),
            ("保留天数 (天):", "retention_days", rec["retention_days"]),
            ("JPEG 质量 (仅 jpg):", "camera_jpeg_quality", rec.get("camera_jpeg_quality", 90)),
        ]
        self.rec_entries = {}
        for i, (label, key, default) in enumerate(params):
            ttk.Label(lf2, text=label).grid(row=i, column=0, sticky="w", pady=3)
            e = ttk.Entry(lf2, width=12)
            e.grid(row=i, column=1, sticky="w", pady=3, padx=4)
            e.insert(0, str(default))
            self.rec_entries[key] = e
        i_fmt = len(params)
        ttk.Label(lf2, text="图片格式:").grid(row=i_fmt, column=0, sticky="w", pady=3)
        self.combo_format = ttk.Combobox(lf2, values=["png", "jpg"], width=8, state="readonly")
        self.combo_format.grid(row=i_fmt, column=1, sticky="w", padx=4, pady=3)
        self.combo_format.set(rec.get("camera_format", "png"))
        ttk.Button(lf2, text="保存录制参数", command=self._save_rec_settings).grid(row=i_fmt + 1, column=0, columnspan=2, pady=(10, 0))

        lf3 = ttk.LabelFrame(tab, text="摄像头配置", padding=10)
        lf3.grid(row=row, column=0, sticky="ew", pady=(0, 8)); row += 1
        self.cam_entries = {}
        cams = rec.get("cameras", {})
        for i, (name, cfg) in enumerate(cams.items()):
            ttk.Label(lf3, text=f"{name}:").grid(row=i, column=0, sticky="w", pady=3)
            e = ttk.Entry(lf3, width=70)
            e.grid(row=i, column=1, sticky="ew", pady=3, padx=4)
            e.insert(0, cfg.get("rtsp", ""))
            self.cam_entries[name] = e
        ttk.Button(lf3, text="保存摄像头配置", command=self._save_cam_settings).grid(row=len(cams), column=0, columnspan=2, pady=(10, 0))
        lf3.columnconfigure(1, weight=1)

        lf4 = ttk.LabelFrame(tab, text="录制时间表", padding=10)
        lf4.grid(row=row, column=0, sticky="nsew", pady=(0, 8)); row += 1
        tab.rowconfigure(row - 1, weight=1)
        tab.columnconfigure(0, weight=1)
        f_day = ttk.Frame(lf4)
        f_day.pack(fill="x", pady=(0, 4))
        days_cn = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]
        self.combo_day = ttk.Combobox(f_day, values=days_cn, width=12, state="readonly")
        self.combo_day.set(days_cn[date.today().weekday()])
        self.combo_day.pack(side="left")
        self.combo_day.bind("<<ComboboxSelected>>", lambda e: self._refresh_timetable())
        self.lbl_tt_type = ttk.Label(f_day, text="", foreground="orange")
        self.lbl_tt_type.pack(side="left", padx=(12, 0))
        self.text_timetable = scrolledtext.ScrolledText(lf4, width=60, height=10, wrap="none", font=("Consolas", 10))
        self.text_timetable.pack(fill="both", expand=True, pady=(0, 6))
        bf = ttk.Frame(lf4)
        bf.pack(fill="x")
        ttk.Button(bf, text="保存时间表", command=self._save_timetable).pack(side="left", padx=4)
        ttk.Button(bf, text="设为今日临时时间表", command=self._set_temp_timetable).pack(side="left", padx=4)
    ...
    def _build_about_tab(self):
        ...
        for i, (k, v) in enumerate(info):
            ttk.Label(tab, text=k + ":", font=("", 10, "bold")).grid(row=i, column=0, sticky="nw", pady=4, padx=(0, 12))
            ttk.Label(tab, text=v).grid(row=i, column=1, sticky="nw", pady=4)

    # ======================== 服务器操作 ========================
    def _start_server(self):
        if self.server.is_running(): return
        self.server.start()
    ...
    def _save_server_settings(self):
        ...
        try:
            ...
            messagebox.showerror("错误", "端口必须是数字"); return
        ...
        messagebox.showinfo("成功", "服务器设置已保存（部分设置需重启服务生效）")

    # ======================== 自启动操作 ========================
    def _refresh_autostart(self): self.var_autostart.set(AutoStart.is_enabled())

    def _on_autostart_toggle(self):
        if self.var_autostart.get():
            if AutoStart.enable():
                messagebox.showinfo("成功", "开机自启动已开启。\n下次开机后程序将自动启动并运行服务器。")
            else:
                messagebox.showerror("失败", "设置自启动失败，请检查系统权限。")
                self.var_autostart.set(False)
        else:
            if AutoStart.disable(): messagebox.showinfo("成功", "开机自启动已关闭。")
            else: messagebox.showerror("失败", "取消自启动失败。")

    # ======================== 用户操作 ========================
    def _refresh_user_list(self):
        self.tree_users.delete(*self.tree_users.get_children())
        for u in self.config.load_users(): self.tree_users.insert("", "end", values=(u,))

    def _add_user(self):
        name = self.entry_new_user.get().strip()
        if not name: messagebox.showwarning("提示", "请输入用户名"); return
        ok, msg = self.config.add_user(name)
        if ok:
            self.entry_new_user.delete(0, "end"); self._refresh_user_list()
            messagebox.showinfo("成功", msg)
        else: messagebox.showerror("失败", msg)

    def _delete_user(self):
        sel = self.tree_users.selection()
        if not sel: messagebox.showwarning("提示", "请先选择一个用户"); return
        username = self.tree_users.item(sel[0])["values"][0]
        ok, msg = self.config.delete_user(username)
        if ok: self._refresh_user_list(); messagebox.showinfo("成功", msg)
        else: messagebox.showerror("失败", msg)
    ...
    def _reset_password(self):
        ...
        if ok: messagebox.showinfo("成功", msg)
        else: messagebox.showerror("失败", msg)

    # ======================== 录制操作 ========================
    def _start_recorder(self):
        self.recorder.start(); self.root.after(500, self._refresh_status)

    def _stop_recorder(self):
        self.recorder.stop(); self.root.after(500, self._refresh_status)

    def _save_rec_settings(self):
        rec = self.config.settings["recorder"]
        try:
            for key, entry in self.rec_entries.items(): rec[key] = type(rec.get(key, 1))(entry.get())
            rec["camera_format"] = self.combo_format.get()
            self.config.save_settings(); messagebox.showinfo("成功", "录制参数已保存")
    ...
    def _save_cam_settings(self):
        ...
        for name, entry in self.cam_entries.items():
            if name in cams: cams[name]["rtsp"] = entry.get().strip()
        self.config.save_settings(); messagebox.showinfo("成功", "摄像头配置已保存")
    ...
    def _refresh_timetable(self):
        ...
        if day == days_cn[date.today().weekday()]:
            temp_path = os.path.join(self.config.timetable_dir, f"{today_str}.txt")
            if os.path.exists(temp_path):
                self.lbl_tt_type.config(text=f"⚠ 今日有临时时间表 ({today_str}.txt)，将优先使用")
                path = temp_path
            else:
                self.lbl_tt_type.config(text=""); path = os.path.join(self.config.timetable_dir, f"{day}.txt")
        else:
            self.lbl_tt_type.config(text=""); path = os.path.join(self.config.timetable_dir, f"{day}.txt")
            
        self.text_timetable.delete("1.0", "end")
        if os.path.exists(path):
            with open(path, "r", encoding="utf-8") as f: self.text_timetable.insert("1.0", f.read())
        else:
            self.text_timetable.insert("1.0", f"# {day}.txt\n# 格式: HH:MM-HH:MM 课程名\n# 例如:\n# 07:50-08:30 英语\n# 08:40-09:20 语文\n")
    ...
    def _save_timetable(self):
        ...
        messagebox.showinfo("成功", f"时间表已保存到 {os.path.basename(path)}")

    def _set_temp_timetable(self):
        content = self.text_timetable.get("1.0", "end").strip()
        if not content.strip() or content.strip().startswith("#"):
            messagebox.showwarning("提示", "时间表内容为空"); return
        today_str = date.today().strftime("%Y-%m-%d")
        path = self.config.save_timetable(content, today_str)
        messagebox.showinfo("成功", f"已设置 {today_str} 的临时时间表。\n该文件将在明日自动清理。")

    # ======================== 状态刷新 ========================
    def _refresh_status(self):
        try:
            if self.server.is_running():
                self.server_status_var.set(f"● 运行中 (端口 {self.config.http_port})")
                self.btn_start_server.config(state="disabled"); self.btn_stop_server.config(state="normal")
            else:
                self.server_status_var.set("● 未启动")
                self.btn_start_server.config(state="normal"); self.btn_stop_server.config(state="disabled")
            ...
            if self.recorder.is_running():
                ...
                if tt_file and os.path.exists(tt_file):
                    ...
                    for start_dt, end_dt, _ in slots:
                        if start_dt <= now <= end_dt: is_actually_recording = True; break
                if is_actually_recording:
                    cams = self.config.settings["recorder"]["cameras"]
                    active = sum(1 for c in cams.values() if c.get("enabled", True))
                    self.rec_status_var.set(f"● 录制中 (多媒体 + {active}路摄像头)")
                else:
                    self.rec_status_var.set("● 待命中 (等待上课时间或无时间表)")
            ...
            self.status_var.set(f"服务器: {srv} | 录制: {rec} | 别名: {self.config.alias}")
        except tk.TclError: pass
